...
const CropSuggestionHistorySchema = new Schema<ICropSuggestionHistory>(
  {
    ...
    input: {
      ...
      purpose: { type: String, enum: ['eat', 'sell', 'decor', 'educational', 'mixed'], required: true },
      sunlight: { type: String, enum: ['full', 'partial', 'shade'], required: true },
      soilType: { type: String, enum: ['loamy', 'sandy', 'clayey', 'silty', 'peaty', 'chalky', 'unknown'], required: true },
